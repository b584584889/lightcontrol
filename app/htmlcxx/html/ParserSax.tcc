...
void htmlcxx::HTML::ParserSax::parse(_Iterator begin, _Iterator end, int special_char)
{
	// std::cerr << "Parsing iterator" << std::endl;
	parse(begin, end, typename std::iterator_traits<_Iterator>::iterator_category(), special_char);
}

template <typename _Iterator>
void htmlcxx::HTML::ParserSax::parse(_Iterator &begin, _Iterator &end, std::forward_iterator_tag, int special_char)
{
...
//						DEBUGP("Parsing beginning of tag\n");
						...
						this->parseHtmlTag(c, d, special_char);
...
template <typename _Iterator>
void htmlcxx::HTML::ParserSax::parseHtmlTag(_Iterator b, _Iterator c, int special_char)
{
	...
	if (special_char == 1)
	{
		while (name_end != c && ( isalnum(*name_end) || (*name_end == ':') ))
	    {
			++name_end;
		}
	}
