 ...
 */
template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE = 65535, int MAX_MESSAGE_HANDLERS = 5>
class Client
{
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::readPacket(Timer& timer)
{
    ...
    //printf("MQTT::Client<>::readPacket() rc[%d] \n", rc);

    if (rc != 1 )
    {
        //printf("MQTT::Client<>::readPacket() rc != 1\n");
    ...
    //printf("MQTT::Client<>::readPacket() len[%d] rem_len[%d] MAX_MQTT_PACKET_SIZE[%d]\n", len, rem_len, MAX_MQTT_PACKET_SIZE);

    if (rem_len > (MAX_MQTT_PACKET_SIZE - len))
    {
        printf("MQTT::Client<>::readPacket() BUFFER_OVERFLOW\n");
    ...
    if (rem_len > 0)
    {
        int iread = ipstack.read(readbuf + len, rem_len, timer.left_ms());
        if ( iread != rem_len )
        {
            printf("MQTT::Client<>::readPacket() len[%d] rem_len[%d] iread[%d]\n", len, rem_len, iread);
            printf("MQTT::Client<>::readPacket() read the rest of the buffer using a callback to supply the rest of the data!!! exit\n");
            goto exit;
        }
    }
    ...
    //printf("MQTT::Client<>::readPacket() rc[%d] this->keepAliveInterval[%d]\n", rc, this->keepAliveInterval);
...
bool MQTT::Client<Network, Timer, a, b>::isTopicMatched(char* topicFilter, MQTTString& topicName)
{
    ...
    //printf("MQTT::Client<>::isTopicMatched() topicFilter[%s] topicName[%s]\n", topicFilter, topicName.lenstring.data);
...
int MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::deliverMessage(MQTTString& topicName, Message& message)
{
    ...
    //printf("MQTT::Client<>::deliverMessage() \n");
...
int MQTT::Client<Network, Timer, a, b>::yield(unsigned long timeout_ms)
{
    ...
    timer.countdown_ms((int)timeout_ms);
    while (!timer.expired())
    {
        //printf("MQTT::Client<>::yield() !timer.expired()\n");
        if (cycle(timer) < 0)
        {
            rc = FAILURE;
            printf("MQTT::Client<>::yield() expired() rc[%d]\n\n\n\n", rc);
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::cycle(Timer& timer)
{
    ...
    switch (packet_type)
    {
        default:
            ...
            rc = packet_type;
            vilslog::printf("MQTT::Client<>::cycle() no more data to read, unrecoverable. Or read packet fails due to unexpected network error\n");
            vilslog::printf("MQTT::Client<>::cycle() rc[%d]\n", rc);
            goto exit;
    ...
    if (keepalive() != SUCCESS)
    {
        ...
        rc = FAILURE;
        vilslog::printf("MQTT::Client<>::cycle() keepalive() != SUCCESS rc = FAILURE\n");
    }
...
exit:
    ...
    {
        rc = FAILURE;
        vilslog::printf("MQTT::Client<>::cycle() end isconnected[%d] closeSession()\n", isconnected);
        vilslog::printf("MQTT::Client<>::cycle() end rc[%d]\n", rc);
        closeSession();
    }


    //printf("MQTT::Client<>::cycle() end rc[%d]\n", rc);
...
int MQTT::Client<Network, Timer, a, b>::waitfor(int packet_type, Timer& timer)
{
    ...
    do
    {
        ...
        //printf("MQTT::Client<>::waitfor() packet_type [%d] rc[%d]\n", packet_type, rc);
    }
    while (rc != packet_type && rc >= 0);

    //printf("MQTT::Client<>::waitfor() end [%d] rc[%d]\n", packet_type, rc);
...
{
    Timer connect_timer((int)command_timeout_ms);
    int rc = FAILURE;
    int len = 0;
...
{
    int rc = FAILURE;
    Timer timer((int)command_timeout_ms);
    int len = 0;
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName, void* payload, size_t payloadlen, unsigned short& id, enum QoS qos, bool retained)
{
    ...
    len = MQTTSerialize_publish(sendbuf, MAX_MQTT_PACKET_SIZE, 0, qos, retained, id,
              topicString, (unsigned char*)payload, (int)payloadlen);
    if (len <= 0)
        goto exit;
...
{
    int rc = FAILURE;
    Timer timer((int)command_timeout_ms);     // we might wait for incomplete incoming publishes to complete
