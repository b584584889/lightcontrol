...
void TcpMgr::Start(const MqInfo& mqInfo, RmqMgrBase *pRmqMgrBase)
{
	...
	m_sock.async_connect(ep, m_strand.wrap(boost::bind(&TcpMgr::OnConnect, shared_from_this(), boost::placeholders::_1)));
...
void TcpMgr::StartRecv(int32_t total_bytes)
{
	if (total_bytes > 0)
	{
		...
		boost::asio::async_read(m_sock, boost::asio::buffer(m_recv_buffer), m_strand.wrap(boost::bind(&TcpMgr::HandleRecv, shared_from_this(), boost::placeholders::_1, boost::placeholders::_2)));
	}
	else
	{
		m_recv_buffer.resize(kReceiveBufferSize);
		m_sock.async_read_some(boost::asio::buffer(m_recv_buffer), m_strand.wrap(boost::bind(&TcpMgr::HandleRecv, shared_from_this(), boost::placeholders::_1, boost::placeholders::_2)));
	}
}
...
void TcpMgr::StartSend()
{
	...
	{
		boost::asio::async_write(m_sock, boost::asio::buffer(*m_pending_sends.front().get()), m_strand.wrap(boost::bind(&TcpMgr::HandleWrite, shared_from_this(), boost::placeholders::_1)));
	}
}
