...
//
// https://github.com/mattdh666/rpi-http-request
//
                              ...
                              int sizeOfBody)
{
  ...
  //printf("HttpRequest::sendRequest()  1 \n");

  initRequest(method, url);

  //printf("HttpRequest::sendRequest()  2\n");
  if (body && !hasContentLength)
  {
    addHeader("Content-Length", sizeOfBody);
  }

  //printf("HttpRequest::sendRequest()  3 \n");
  if (headers)
  {
    const char **itr = headers;
  ...
  //printf("HttpRequest::sendRequest()  4 \n");

  sendHeaders();
  //printf("HttpRequest::sendRequest()  5 \n");
...
void HttpRequest::processRequest()
{
  ...
  int bytesReceived = (int)recv(_socket, (char*)data, sizeof(data), 0);
...
void HttpRequest::initSocket()
{
  ...
  inet_ntop(AF_INET, &(address.sin_addr), buffer, len);
  //printf("address:%s\n",buffer);
...
void HttpRequest::send(const unsigned char *data, int sizeOfData)
{
  if (_socket < 0)
  {
    //printf("HttpRequest::send() 1\n");
    initSocket();
  }

  //printf("HttpRequest::send() 2\n");
  while (sizeOfData > 0)
  {
    int bytesSent = (int)::send(_socket, data, sizeOfData, 0);
