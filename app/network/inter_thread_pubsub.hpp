...
namespace ITPS {
    ...
    class MsgChannel {
        ...
        public:
            ...
            MsgChannel(std::string topic_name, std::string msg_name)
            {
                ...
                //std::cout << "MsgChannel() this->key:" << this->key<< std::endl;
            ...
            {
                //std::cout << "MsgChannel::get_channel() topic_name:" << topic_name<< std::endl;

                ITPS_reader_lock(table_mutex);
                std::string key = topic_name + "." + msg_name;
                //std::cout << "MsgChannel::get_channel() key:" << key<< std::endl;
                // if key doesn't exist
                if(msg_table.find(key) == msg_table.end())
                {
                    //std::cout << "MsgChannel::get_channel() return nullptr"<< std::endl;

                    return nullptr;
                }
                //std::cout << "MsgChannel::get_channel() return "<< msg_table[key]<<std::endl;
    ...
    class Publisher {
        public:
            ...
            {
                MsgChannel<Msg> * channelExisted = ITPS::MsgChannel<Msg>::get_channel(topic_name, msg_name);
                if(channelExisted == nullptr)
                {
                    channel = boost::shared_ptr<ITPS::MsgChannel<Msg>>(new ITPS::MsgChannel<Msg>(topic_name, msg_name));
                }
                else
                {
                    channel = boost::shared_ptr<ITPS::MsgChannel<Msg>>(channelExisted);
                }
            }
    ...
    class Subscriber {
        public:
            ...
            Subscriber(std::string topic_name, std::string msg_name, unsigned int queue_size)
                ...
                {
                    //std::cout << "Subscriber::Subscriber() " << topic_name << msg_name << queue_size << std::endl;

                msg_queue = boost::shared_ptr<ConsumerProducerQueue<Msg>>(
                    new ConsumerProducerQueue<Msg>(queue_size)
                );
                //std::cout << "msg_queue:" << msg_queue << std::endl;
                use_msg_queue = true;
                //std::cout << "use_msg_queue:" << use_msg_queue << std::endl;
            ...
            bool subscribe() {
                ...
                //std::cout << "subscribe() " << topic_name<<" "<<msg_name<< std::endl;

                channel = MsgChannel<Msg>::get_channel(topic_name, msg_name);
                if(channel == nullptr)
                {
                    // std::cout << "subscribe() channel == nullptr " << channel<< std::endl;

                    return false;
                }
                if(use_msg_queue)
                {
                    // std::cout << "subscribe() add_msg_queue " << msg_queue<< std::endl;
