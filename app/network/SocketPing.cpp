...
{
    vilslog::printf("------statistics------\n");
    vilslog::printf("%d packets transmitted, %d received , %%%d lost\n", m_nSend, m_nRecv,
...
bool CPing::CreateSocket()
{
    ...
    {
        vilslog::printf("CreateSocket: getprotobyname failed:%d\n",errno);


        return false;
    }
    if (getuid())
        m_nSocketfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_ICMP);
    else
        m_nSocketfd = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);

    if (m_nSocketfd < 0)
    {
        vilslog::printf("CreateSocket: create socket failed:%d\n",errno);
        return false;
    }
    ...
    if(nRet != 0)
    {
        vilslog::printf("CreateSocket: set socket receive buf failed:%d\n",errno);
    ...
    {
        //int nret;
        host = gethostbyname(m_strIp.c_str());

        // gethostbyname_r(m_strIp.c_str(), &hostinfo, buf, sizeof(buf), &host, &nret);
        // if (nret != 0) /*是主机名*/
        // {
        //     vilslog::printf("CreateSocket: gethostbyname error %s failed:%d\n",m_strIp.c_str(),errno);
        //
        //     return false;
        //
        // }
...
bool CPing::ping(int times)
{
    ...
    while(i < times)
    {
        ...
        if(!bRet)
        {
            vilslog::printf("ping: create socket falied!\n");
        ...
        // vilslog::printf("PING %s(%s): %d bytes data in ICMP packets.\n", m_strIp.c_str(),
...
void CPing::send_packet()
{
    ...
    while (m_nSend < m_nMaxTestpkg)
    {
        ...
        {
            // vilslog::printf("send_packet: send error :%d\n",errno);
...
void CPing::recv_packet()
{
    ...
    while(m_nRecv < m_nSend)
    {
        ...
        if(n == 0)
        {
            // vilslog::printf("recv_packet: select time out :%d",errno);

            return ;
        }
        else if(n < 0)
        {
            // vilslog::printf("recv_packet: select error :%d",errno);
        ...
        if ((n = (int)recvfrom(m_nSocketfd, m_recvpacket, sizeof (m_recvpacket), 0,
                    (struct sockaddr *) &m_from, (socklen_t *)&fromlen)) <= 0)
        {
            // vilslog::printf("recv_packet: recv error :%d",errno);
...
int CPing::unpack(char *buf,int len)
{
    ...
    {
        vilslog::printf( "ICMP packets\'s length is less than 8");
    ...
    if ((icmp->icmp_type == ICMP_ECHOREPLY) && (icmp->icmp_id == m_Pid)&&(m_Ip == inet_ntoa(m_from.sin_addr)))
    {
        ...
        /*显示相关信息*/
        // vilslog::printf("%d\tbyte from %s\t: icmp_seq=%u\tttl=%d\trtt=%.3f\tms\n",
        //         len,
    ...
    {
       // vilslog::printf("throw away the old package %d\tbyte from %s\t: icmp_seq=%u\tttl=%d\trtt=%.3f\tms",
       //          len,
       //          inet_ntoa(m_from.sin_addr),
       //          icmp->icmp_seq,
       //          ip->ip_ttl,
