...
function create_user_record_table_item(ui_index, record, user_record)
{
    ...
    {
        Spanuserid.setAttribute("style","font-size:14px; font-weight:bold; color:#330033;");
        Spanuserid.textContent = getUseName(userid);
    }
    row.cells[cellIndex].appendChild(Spanuserid);
    cellIndex++;
    ...
    for (var i = 0; i < user_record.card.length; ++i)
    {
        ...
        {
            divpresenttime.appendChild(create_present_label(user_card.updatetime));
        }
    }
    cellIndex++;
...
function open_userrollcallrecord_page()
{
    var win = window.open('/index.php/vilsuserrollcallrecord/'+project_id, '_blank');
    win.focus();
}
