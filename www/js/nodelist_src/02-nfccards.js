...
var nfccards_PreLoadIndex = -1;
var nfccards_CurrentLoadIndex = 0;
var LOAD_STATE_END = 9999;
var nfccards_glCurrentPage = 1;

var gl_OnLinelist_nfccards = [];
var gl_notOnLinelist_nfccards = [];

check_nfccards_loading_status();

function check_nfccards_loading_status()
{
    switch(nfccards_CurrentLoadIndex)
    {
        case 0:
            if ( nfccards_PreLoadIndex != nfccards_CurrentLoadIndex )
            {
                // console.log("check_nfccards_loading_status() nfccards_PreLoadIndex:" + nfccards_PreLoadIndex +" != nfccards_CurrentLoadIndex:" + nfccards_CurrentLoadIndex);
                nfccards_PreLoadIndex = nfccards_CurrentLoadIndex;
        ...
        case 1:
            if ( nfccards_PreLoadIndex != nfccards_CurrentLoadIndex )
            {
                ...
                nfccards_CurrentLoadIndex = nfccards_CurrentLoadIndex + 1;
            }
        break;
        case 2:
            if ( nfccards_PreLoadIndex != nfccards_CurrentLoadIndex )
            {
                ...
                nfccards_CurrentLoadIndex = LOAD_STATE_END;
            }
        break;
    }

    if ( nfccards_CurrentLoadIndex != LOAD_STATE_END )
        setTimeout(check_nfccards_loading_status, 100);
    else
    {
        // console.log("check_nfccards_loading_status() Stop.");

        setTimeout(reload_nfccard_info, 10000);

        // if (gl_nfccard_reload == 0)
        //     setTimeout(reload_nfccard_info, 3000);
        // else
        //     setTimeout(reload_nfccard_info, 30000);
        // gl_nfccard_reload += 1;
    }
}
...
function load_nfccard()
{
    ...
    if ( gl_activetab === 'activetab_nfccards' )
    {
    }
    else
    {
        setTimeout(load_nfccard, 1000);
        return;
    }

    var offset = (nfccards_glCurrentPage - 1) * LOAD_PAGE_NUM;
    var count = LOAD_PAGE_NUM;
    var showonline = gl_showfilter_showonline ? 1:0;
    var showoffline = gl_showfilter_showoffline ? 1:0;

    gl_OnLinelist_nfccards = [];
    gl_notOnLinelist_nfccards = [];
    ...
    var p = $.getJSON(targetURL, 'loadcard=1&s='+offset+'&count='+count+'&project_id='+project_id+'&showonline='+showonline+'&showoffline='+showoffline, function(data)
    {
        ...
        {
            if (device.alive === '0')
                gl_notOnLinelist_nfccards.push(device);
            else
                gl_OnLinelist_nfccards.push(device);
        });
    })
    .success(function() {
        nfccards_CurrentLoadIndex = nfccards_CurrentLoadIndex + 1;
...
function draw_nfccard_table()
{
    var record_table = document.getElementById('NFC_CARD_TABLE');
    ...
    {
        for (var i = 0; i < gl_OnLinelist_nfccards.length; ++i)
        {
            var card = gl_OnLinelist_nfccards[i];

            var cardIndex = (nfccards_glCurrentPage - 1) * LOAD_PAGE_NUM;
            var row = nfccards_create_record_table_item(cardIndex + i, card);
            record_table.appendChild(row);
        }
        uiindex = gl_OnLinelist_nfccards.length;
    }
    if (gl_showfilter_showoffline)
    {
        for (var i = 0; i < gl_notOnLinelist_nfccards.length; ++i)
        {
            var card = gl_notOnLinelist_nfccards[i];

            var cardIndex = (nfccards_glCurrentPage - 1) * LOAD_PAGE_NUM + uiindex;
            var row = nfccards_create_record_table_item(cardIndex + i, card);
            record_table.appendChild(row);
        }
    }
...
function nfccards_create_record_table_item(ui_index, card)
{
    ...
    if (edit_node_info)
    {
        ...
        linkEdit.setAttribute("onclick","edit_nfccard_name(\""+ card.cardid +"\",\""+ ShowName+"\");return false;");
    ...
    if (edit_node_info)
    {
        ...
        deleteBtn.setAttribute("onclick","confirm_delete_nfccard(\""+card.cardid+"\");return false;");
...
function edit_nfccard_name(cardid, cardname)
{
    console.log("edit_nfccard_name() cardid:"+cardid+" cardname:"+cardname);

    var newValue= prompt("編輯NFC卡片名稱:", cardname);
    if ( newValue )
    {
        ...
        update_nfccard_name(cardid, newValue);
    }
}

// function edit_card(cardid)
// {
//     console.log("edit_card() cardid:"+cardid);
//
//     var editBtn = document.getElementById("uiid_editbtn_"+cardid);
//     editBtn.setAttribute("title","Save this Card");
//     editBtn.setAttribute("onclick","save_card(\""+cardid+"\");return false;");
//     var imgEdit = document.getElementById("uiid_editimg_"+cardid);
//     imgEdit.setAttribute("class","fa fa-save");
//
//     var deleteBtn = document.getElementById("uiid_deletebtn_"+cardid);
//     deleteBtn.setAttribute("style","display:block");
// }

// function save_card(cardid)
// {
//     setTimeout(reload_nfccard_info, 300);
// }
...
function update_nfccard_name(cardid, newValue)
{
    ...
    jsondata['update_nfccard_name'] = 1;
...
//     var jsondata = {};
//     jsondata['project_id'] = ''+project_id;
//     jsondata['create_new_nfccard'] = 1;
//     jsondata['cardid'] = input_id;
//
//     var targetURL = gl_target_server + "/php/nfclocator.php";
//
//     jQuery.ajax({
...
function confirm_delete_nfccard(cardid)
{
    if (confirm('Are you sure you want to delete this card:'+cardid+'   ?'))
    {
        // delete it!
        delete_nfccard(cardid);
    }
    else
    {
        // Do nothing!
...
function delete_nfccard(cardid)
{
    var jsondata = {};
    jsondata['project_id'] = ''+project_id;
...
$("#scannewnfccardbutton").click(function()
{
    var jsondata = {};
    jsondata['project_id'] = ''+project_id;
    jsondata['scan_nfccard'] = 1;
...
function reload_nfccard_info()
{
    nfccards_PreLoadIndex = -1;
    nfccards_CurrentLoadIndex = 0;
    check_nfccards_loading_status();
}

// function loc_tab_uhfcards_change()
// {
//     setTimeout(reload_nfccard_info(), 10);
// }

function nfccards_move_page_backward()
{
    var current_page_num_ui = document.getElementById('nfccards_id_current_page_num');

    if ( nfccards_glCurrentPage <= 1 )
    {
        // do nothing
    }
    else
    {
        nfccards_glCurrentPage--;
        current_page_num_ui.textContent = nfccards_glCurrentPage;

        reload_nfccard_info();
    }
}

function nfccards_move_page_forward()
{
    var current_page_num_ui = document.getElementById('nfccards_id_current_page_num');
...
function open_nfccards_page()
{
    var win = window.open('/index.php/vilsnfccards/'+project_id, '_blank');
    win.focus();
}
